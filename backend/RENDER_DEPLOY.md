...
| `SMTP_EMAIL` / `SMTP_APP_PASSWORD` | Required for registration/password-reset OTP emails to actually send. A Gmail address, plus a 16-character **App Password** generated at myaccount.google.com/apppasswords (needs 2-Step Verification enabled first — your normal Gmail password won't work here). If these are left blank, `app/services/email.py` silently skips sending and only logs a warning — the API still reports `"OTP sent successfully"`, so this is easy to miss until a user says emails never arrive. |
