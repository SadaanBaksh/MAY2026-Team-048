...
| INT-006 | `POST /api/v1/tickets/` -> `PATCH /api/v1/tickets/{id}` | Resident attempts to change a newly created `Pending` ticket directly to `Closed` with rating `5` | `409 Conflict` with `Only resolved tickets can be closed`; ticket remains Pending | `409 Conflict` returned with the expected message | Success | `test_int_006_resident_cannot_close_ticket_before_resolution` |
| INT-007 | Create public service -> employee assignment -> maintenance progress/resolution -> history | Resident report, active employee and worker, resolution remarks | Worker sees only the assigned job; lifecycle reaches `Resolved`; history records Pending -> Assigned -> In_Progress -> Resolved | All API calls returned `200/201`; employee/worker notifications and four history states matched | Success | `test_int_007_public_service_assignment_resolution_and_history_workflow` |
| INT-008 | Create/send notice -> resident notice feeds -> notification read | Manager notice targeted to Tower N; residents in Tower N and Tower S | Draft is initially hidden; only Tower N receives the sent notice and unread notification; notification can be marked read | Target resident received the notice and notification; non-target resident received neither; read state updated | Success | `test_int_008_targeted_notice_delivery_and_notification_workflow` |
| INT-009 | Send forgot-password OTP -> verify OTP -> reset password -> login | Registered resident, captured OTP, new password | OTP yields reset token; old password is rejected; new password authenticates | Reset completed; old login returned `401`; new login returned `200` | Success | `test_int_009_forgot_password_otp_reset_and_login_workflow` |
| INT-010 | Resident chat -> chat history -> clear history | Resident message and deterministic mocked AI reply | Resident and assistant turns persist in order with suggestions; delete empties history | Two turns and suggestions matched; `204` delete left an empty history | Success | `test_int_010_ai_chat_persists_and_clears_conversation` |
| INT-011 | Create ticket -> resident cancellation -> history and employee notifications | Newly created Pending ticket cancelled by its resident | Ticket becomes Cancelled; history is Pending -> Cancelled; employee receives cancellation notification | Status, two history entries, and cancellation notification matched | Success | `test_int_011_resident_cancellation_updates_history_and_employee_notification` |
